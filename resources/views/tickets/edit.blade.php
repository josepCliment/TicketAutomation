                ...
                @if ($ticket->products->count() > 0)
                    <div class="space-y-4 bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                        @foreach ($ticket->products as $index => $product)
                            <div class="bg-white dark:bg-gray-800 p-4 rounded border border-gray-200 dark:border-gray-600 flex justify-between items-start">
                                <div class="flex-1">
                                ...
                                <form method="POST" action="{{ route('products.destroy', [$ticket, $product->id]) }}" style="display: inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" onclick="return confirm('Are you sure you want to delete this product?')" class="ml-4 px-3 py-2 bg-red-600 hover:bg-red-700 text-white text-sm rounded-lg font-medium transition whitespace-nowrap">
                                        Delete
                                    </button>
                                </form>
                            </div>
